<?php
// This part is the front page of the user timetable.
...
<body class="loggedin">
    <div class="container-fluid no-padding">
        ...
        <div class="container mb-5">
            <div class="content col" id="display">
                <h2>Timetable</h2>
                <hr>
                <h4>Setting</h4>
                <button type="button" style="position:relative; top:0px; left:20px;" class="bn632-hover bn26" onclick="privateCourse(<?php echo $_SESSION['id']; ?>)">Set to Private</button>
                <button type="button" style="position:relative; top:0px; left:40px;" class="bn632-hover bn27" onclick="publicCourse(<?php echo $_SESSION['id']; ?>)">Set to Public</button>
                <hr>
                <h4>Request and Sharing</h4>
                <?php
                $stmt = $db->prepare("SELECT post_text, isShow, course_id FROM home_post WHERE id = ?");
                $stmt->bind_param('i', $_SESSION['id']);
                $stmt->execute();
                $stmt->store_result();
                $stmt->bind_result($post_text, $isShow, $course_id);
                $stmt->fetch();
                $stmt->close();
                if ($isShow == 1) {
                    echo 'Your post info:<p>';
                    echo $post_text;
                    $stmt = $db->prepare("SELECT courseCode, courseTitle FROM courses WHERE course_id = ?");
                    $stmt->bind_param('i', $course_id);
                    $stmt->execute();
                    $stmt->store_result();
                    $stmt->bind_result($courseCode, $courseTitle);
                    $stmt->fetch();
                    $stmt->close();
                    echo '</p>Your willing course ID:<p>';
                    echo $courseCode;
                    echo " ";
                    echo $courseTitle;
                } else {
                    echo 'No post to public, if you want to push a post, you may click push and edit.</p>';
                }
                ?>
                <form action="push_post.php" method="post">
                    <div class="form-group">
                        <label for="post_text">*Enter your post description below:</label> 
                        <textarea class="form-control" placeholder="Enter your text" id="post_text" rows="3" required></textarea>
                        <p>*Required</p>
                    </div>
                    <div class="form-group">
                        <label for="courselist">Choose Course</label>
                    ...
                    <button type="submit" style="position:relative; top:0px; left:20px;" class="bn632-hover bn26">Push and edit</button>
                    <button type="submit" style="position:relative; top:0px; left:40px;" class="bn632-hover bn27" id="close_post" value="1">Close post</button>
                </form>
